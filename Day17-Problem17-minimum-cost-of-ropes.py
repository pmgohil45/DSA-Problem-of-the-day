...
class Solution:
    ...
    def minCost(self, arr: List[int]) -> int:  # Use List[int] instead of list[int]
        # Initialize a min-heap from the given array
        heapq.heapify(arr)
        
        total_cost = 0
        
        # Keep combining ropes until only one remains
        while len(arr) > 1:
